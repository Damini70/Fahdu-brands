...
export default function LoginPage() {
    ...
    const onLogin = async () => {
        try {
            setLoading(true)
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/brand/login`, user)
            if(response.data){
    ...
    return (
        ...
        <div className="container login_section flex items-center justify-between max-w-[1280px] h-[100vh] m-auto max-md:justify-center">
            ...
            <div className="w-[507px]">
                <h1 className="login_title">{loading ? "Login Processing" : "Login"}</h1>
                <p className="login_subtitle" >Welcome to Fahdu, Login to Continue...</p>
                <div className="flex flex-col">
                    ...
                    <div className="flex flex-col w-full mb-[15px]">
                        <label className="brands_label" htmlFor="phone">Password*</label>
                        <input className="brands_input"
                        id="password"
                        type="password"
                        value={user.password}
                        ...
                        />
                        <button className="text-[14px] leading-[16px] font-[500] ml-[auto] mt-[5px]">Forgot Password?</button>
                    </div>
                    <div className="flex items-center gap-[10px] mt-[10px] mb-[20px]">
                        <input className="w-[21px] h-[21px] rounded-[6px] cursor-pointer" type="checkbox" />
                        <p className="text-[18px] leading-[21px] font-[500]">Remember me.</p>
                    </div>
                    <button onClick={onLogin}  className="login_button">Log In</button>
                    {/* <div>
                    ...
                    <p className="text-[18px] leading-[21px] font-[500] text-center mt-[20px]">Do you have an account? <Link href={"/signup"} className="text-[#FFA86B] font-[600] hover:underline">SignUp</Link></p>
                </div>
            </div>
        </div>
        </>
