...
const influencerCategory=["Nano (1K - 10K)","Micro (10K - 50K)","Mid Tier(50K to 100K)","Macro (100K to 500K)","Celeb ( 500k+ )"]
export default function CampaignDetails() {
  ...
  const onSubmit = async(data: any) => {
    ...
    if (uploadImage?.data) {
        ...
        const campaignResponse = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL }/api/brand/campaign/create`,
            campaignDetails,
            {
                headers: {
  ...
  return (
    ...
    <div className={style.brands_p_section}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
            <h2 className={`${style.brands_form_title} mb-9`}>Campaign Details</h2>
            <input
...
<select   {...register('influencerCategory', { required: 'Influencer Category is required' })}
            className="brands_input campaign_obj"
            style={{ width: '100%' }}>
    <option value=''>Influencer Categories</option>
    {influencerCategory.map((item,index)=><option value={index+1}>{item}</option>)}
    </select>

        <input
          {...register('creatorInterest', { required: 'Creator Interest is required' })}
          className="brands_input"
          type="text"
        ...
        />
         ...
         <select
            {...register('campaignObjective', { required: 'Campaign Objective is required' })}
            className="brands_input campaign_obj"
            style={{ width: '100%' }}
          >
            ...
            <div className={`${style.brands_enquiry_file} brands_input`} >
                ...
                <div className={`${style.brands_enquiry_file} brands_input`} >
                    <div className={`${style.choose_file_block} mx-3`}>
                        <img src={"icons/brands-add-logo.svg"} alt="add file icon" />
                        <p className="file_text drap-drop-text">Drag & Drop Brand Logo</p>
                        <span className={style.file_divider}>or</span>
                        <div>
                            <input className={style.drag_drop_input} type="file" {...register('brandsLogo',{required: true,validate:{ checkFileSize: (value) => {
              return value && value[0] && value[0].size <= 15 * 1024 * 1024 || toast.error('Image size should not exceed 15 MB');
            }}})}/>
                        </div>
                    </div>
                </div>
            ...
            <div className={`${style.brands_enquiry_file} brands_input`}>
                <select {...register('creatorCount',{ required: true })}><option value="">Select Creators count for the collaboration</option><option value="10 Creators">10 Creators</option><option value="20 Creators">20 Creators</option><option value="50 Creators">50 Creators</option><option value="100 Creators">100 Creators</option><option value="100+ Creators or Custom">100+ Creators or Custom</option></select>
                </div>
